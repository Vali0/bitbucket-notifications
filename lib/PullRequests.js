...
class PullRequests {
    ...
    set options(options) {
        ...
        if (options) {
            opts.regExp = options.regExp || /[a-zA-Z]+-[0-9]+/;
            opts.addJiraLinks = options.addJiraLinks || false;
            opts.jira = options.jira || undefined;
        } else {
            opts.regExp = /[a-zA-Z]+-[0-9]+/;
            opts.addJiraLinks = false;
            opts.jira = undefined;
        }
    ...
    getPullRequests(options, pullRequestsList) {
        ...
        // Checks if there are pull requests from previous recursion. If there are takes them if not - creates new empty object
        let pullRequestsObj = pullRequestsList || {};
        let body = {
            method: 'GET',
        ...
        return request(body)
            .then((data) => {
                try {
                    // Parses pull requests from API response
            ...
            .catch((err) => {
                ...
                return this.bitbucket.refreshTokens()
                    .then(() => this.getPullRequests(options, pullRequestsObj))
                    .catch((responseError) => {
                        throw new Error(`PullRequests: Can not refresh access token. Stack trace: ${responseError}`);
                    });
            });
    }
}
 ...
 *
 * @param {Object} pullRequests - Pull requests from Bitbucket response
 * @param {Object} targetPullRequests - Previously serialzied pull requests
...
function serializePullRequestsData(pullRequests, targetPullRequests) {
    ...
    for (let item in pullRequests) { // eslint-disable-line guard-for-in
 ...
 *
 * @param {String} params - Parameter for search query
