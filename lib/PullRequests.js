        ...
        return request(body)
            .then(data => {
                try {
                    ...
                    // TODO: add a check as there is no point to merge objects if there is only one page
                    // TODO: remove mergeObjects. To improve performance pass pullRequestsObj to serializePullRequestsData function and append PRs there
...
function serializePullRequestsData(pullRequests) {
    ...
    for (let item in pullRequests) {
        ...
        let ticketId = prTitle.match(this.options.regExp); // TODO: this returns array. Usually it will return only one or must show first result. Fix and test
        let jiraUrl;
