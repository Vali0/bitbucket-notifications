...
const browseUrlPattern = 'https://{domain}/browse/{ticketId}';
const transitionUrlPattern = 'https://{domain}/rest/api/2/issue/{issueIdOrKey}/transitions';

class Jira {
    constructor(credentials) {
        this.domain = credentials.domain;
        this.selfHosted = credentials.selfHosted;
        this.username = credentials.username;
        this.authorisationToken = credentials.authorisationToken;

        if (this.domain) {
            if (this.selfHosted === true) {
                ...
                this.browseUrl = browseUrlPattern.replace('{domain}', this.domain + '.atlassian.net');
                this.transitionUrl = transitionUrlPattern.replace('{domain}', this.domain + '.atlassian.net');
            }
        }
    }
